...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        int heigh= (int) (width/2);
        RelativeLayout.LayoutParams params=new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,heigh);
        FrameLayout.LayoutParams paramsf=new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,heigh);
        ...
        text = indicatorLayout.findViewById(R.id.title_viewpager_indicator);
        viewPagerFragmentLayout = (FrameLayout) view.findViewById(R.id.layout_viewager_content);
        viewPagerFragmentLayout.setLayoutParams(paramsf);
...
//        indicatorLayout.setLayoutParams(params);
        indicatorLayout.setVisibility(View.GONE);//隐藏说明
    }
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setImageResource(R.drawable.radius_oval_gray);
            indicators[i].setColorFilter(ColorRgbUtil.getGrayText());
        }
        if (indicators.length > selectedPosition){
            indicators[selectedPosition].setColorFilter(ColorRgbUtil.getBaseColor());
            if (text!=null) text.setText(infos.get(selectedPosition).getContent());
        }

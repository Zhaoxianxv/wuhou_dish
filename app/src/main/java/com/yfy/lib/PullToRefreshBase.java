...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private OnScrollValueListener<T> mOnScrollValueListener;
	...
	// mFooterLayout.reset();
	//
	// mRefreshableView.scrollTo(0, getScrollY());
	// scrollTo(0, 0);
	//
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			if (mIsBeingDragged) {
				...
				setState(State.RESET);
				if (mOnScrollValueListener != null) {
					Orientation direction = getPullToRefreshScrollDirection();
					int value = direction == Orientation.HORIZONTAL ? getScrollX()
							: getScrollY();
					mOnScrollValueListener.onRelease(this, value);
				}
				return true;
			}
			break;
	...
	public final void showLoadingLayout() {
		...
		if (mFooterLayout != null) {
			mFooterLayout.showInvisibleViews();
		}
	}
	...
	public void setOnScrollValueListener(OnScrollValueListener<T> listener) {
		mOnScrollValueListener = listener;
	}
	...
	public void scrollTo(int x, int y) {
		...
		if (mOnScrollValueListener != null) {
			Orientation direction = getPullToRefreshScrollDirection();
			int value = direction == Orientation.HORIZONTAL ? x : y;
			mOnScrollValueListener.onScroll(this, value, mCurrentMode);
		}
	}
	...
	protected void onRefreshing(final boolean doScroll) {
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				case PULL_FROM_END:
					int footSize = getFooterSize();
					if (footSize == 0) {
						smoothScrollTo(-measureFooterLayout(), listener);
					} else {
						smoothScrollTo(footSize, listener);
					}
					break;
				default:
				case PULL_FROM_START:
					if (getHeaderSize() == 0) {
						smoothScrollTo(-mesureHeaderLayout(), listener);
					} else {
	...
	private int mesureHeaderLayout() {
		int w = MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED);
		int h = MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED);
		mHeaderLayout.measure(w, h);
		int height = mHeaderLayout.getMeasuredHeight();

		return height;
	}

	private int measureFooterLayout() {
		int w = MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED);
		int h = MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		LoadingLayout header = defineHeaderLayout(context,
				Mode.PULL_FROM_START, a);
		LoadingLayout footer = defineFooterLayout(context, Mode.PULL_FROM_END,
				a);
		mHeaderLayout = header == null ? createLoadingLayout(context,
				Mode.PULL_FROM_START, a) : header;
		mFooterLayout = footer == null ? createLoadingLayout(context,
				Mode.PULL_FROM_END, a) : footer;

		if (a.hasValue(R.styleable.PullToRefresh_forRefresh)) {
			mForRefresh = a.getBoolean(R.styleable.PullToRefresh_forRefresh,
					true);
			if (!mForRefresh) {
				hideLoadingLayout();
				mMode = Mode.BOTH;
	...
	protected LoadingLayout defineHeaderLayout(Context context, Mode mode,
			TypedArray attrs) {
		return null;
	}

	protected LoadingLayout defineFooterLayout(Context context, Mode mode,
			TypedArray attrs) {
		return null;
	}
	...
	protected void onFinishInflate() {
		...
		clearEdgeFlowDrawable();
	}

	private final void clearEdgeFlowDrawable() {
		if (VERSION.SDK_INT > VERSION_CODES.GINGERBREAD) {
			if (mRefreshableView instanceof AbsListView) {
				clearBlueEdge(AbsListView.class);
			} else if (mRefreshableView instanceof ScrollView) {
				clearBlueEdge(ScrollView.class);
			}
		}
	}

	@TargetApi(9)
	private final void clearBlueEdge(Class<?> clazz) {
		try {
			...
			Object egtObject = egtField.get(mRefreshableView); // this

			Object egbObject = egbBottom.get(mRefreshableView);
	...
	public static interface OnScrollValueListener<V extends View> {
		public void onScroll(final PullToRefreshBase<V> refreshView, int value,
                             Mode mode);

		public void onRelease(final PullToRefreshBase<V> refreshView, int value);
	}
